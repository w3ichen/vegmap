...
class PoseInfoBridge(Node):
    ...
    def timer_callback(self):
        ...
        process = subprocess.Popen(
            ["ign topic -e -t /world/warehouse/pose/info -n1"], 
            shell=True, 
            stdout=PIPE
        )
        ...
        poses = re.findall(r'pose\s*{[^}]*name:\s*"([^"]*)"[^}]*position\s*{([^}]*)}\s*orientation\s*{([^}]*)}', 
                         output, re.DOTALL)
        ...
        if tf_msg.transforms:
            ...
            self.get_logger().info(f'Published {len(tf_msg.transforms)} transforms')
