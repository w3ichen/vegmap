...
namespace vegmap_planner
{
    ...
    void VegmapPlanner::computeShortestPath()
    {
        ...
        if (open_set_.empty() && last_goal_.x >= 0)
        {
            ...
            open_set_.insert(goal_key);
        }

        while (!open_set_.empty() &&
               ((iterations == 0) || // Always do at least one iteration
                (open_set_.begin()->k1 < calculateKey(last_start_).k1 ||
                 getRhsValue(last_start_) != getGValue(last_start_))))
        {
            ...
            StateKey current = *open_set_.begin();
            open_set_.erase(open_set_.begin());
     ...
     */
    double vegmap_planner::VegmapPlanner::getCost(const vegmap_planner::CellIndex & /* from */, const vegmap_planner::CellIndex &to)
    {
    ...
    std::vector<CellIndex> VegmapPlanner::extractPath()
    {
        ...
        while (!(current == last_goal_) && iterations < MAX_ITERATIONS)
        {
            ...
            for (const auto &neighbor : getNeighbors(current))
            {
                ...
                if (total_cost < min_cost && getGValue(neighbor) < getGValue(current))
                {
                    min_cost = total_cost;
                    next_cell = neighbor;
                }
        ...
        RCLCPP_INFO(node_->get_logger(), "[Benchmark] Path extraction time: %ld ms", duration_ms);
        RCLCPP_INFO(node_->get_logger(), "[Benchmark] Path length: %zu cells", path.size());
        // #endif
        ...
        const geometry_msgs::msg::PoseStamped &goal)
    {
        ...
        std::cout << "I AM EXTRACTINGGG PATH!!!!\n";
